...
void system_info()
{
  ...
  Serial.println("Flash Size: " + String(ESP.getFlashChipSize() / 1024) + " kilobytes");                                                                            // Print the flash size
  ...
  Serial.print("Chip Model: " + String(ESP.getChipModel()));                                                                                                        // Print the chip model
  Serial.printf("%d cores Wifi %s%s", chip_info.cores, (chip_info.features & CHIP_FEATURE_BT) ? "/BT" : "", (chip_info.features & CHIP_FEATURE_BLE) ? "/BLE" : ""); // Print the chip features
  Serial.printf("%dMB %s flash", spi_flash_get_chip_size() / (1024 * 1024), (chip_info.features & CHIP_FEATURE_EMB_FLASH) ? "embeded" : "external");                // Print the flash size
  Serial.println("\nSilicon revision:" + chip_info.revision);                                                                                                       // Print the chip revision
